...
node *constructreepre(int pre[],int start,int end)
{
	static int i=0,j;
	if(start>end)
		return NULL;
	...
	temp->left=constructreepre(pre,start+1,pos-1);
	temp->right=constructreepre(pre,pos,end);
	return temp;
}
int check(node *res)
{
...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	node *res=constructreepre(pre,0,4);
	inorder(res);
	int k=check(res);
	if(k)
